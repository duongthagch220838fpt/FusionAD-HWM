...
class HWMNet(nn.Module):
    ...
    def forward(self, x):
        ...
        scale_img = img
        print('scale_img:', scale_img.shape)
        ##### shallow conv #####
        x1 = self.conv_01(img)
        print('x1:', x1.shape)
        encs = []
        ...
        fea_1, fea_2 = list_features[0], list_features[1]
        print('fea_1:', fea_1.shape)
        fea_concat = torch.cat([fea_1, fea_2], dim=1)
        print('fea_concat:', fea_concat.shape)
        return fea_concat
            # print('ms_result:', i.shape)
        # for i in list_features:
        #     print('list_features:', i.shape)
        # # Multi-scale selective feature fusion
        # msff_result = self.final_ff(ms_result)
        # # print('ms_result:', ms_result.shape)    
        # ##### Reconstruct #####
...
if __name__ == "__main__":
    ...
    model = HWMNet(in_chn=3, wf=64, depth=2).cuda()
    out = model(input)
